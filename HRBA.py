...
print str(xlfile)
wb_io = load_workbook(xlfile, data_only=True)
...
nom_R1 = GTC.ureal(val1, val1/1e4, 8, label='nom_R1')

val2 = ws_Rlink['C'+str(RL_start_row+3)].value
assert nom_R1 is not None, 'Missing nominal R2 value!'
nom_R2 = GTC.ureal(val2, val2/1e4, 8, label='nom_R2')

val1 = ws_Rlink['D'+str(RL_start_row+2)].value
assert val1 is not None, 'Missing nominal V1 value!'
abs_V1 = GTC.ureal(val1, val1/1e4, 8, label='abs_V1')

val2 = ws_Rlink['D'+str(RL_start_row+3)].value
assert val2 is not None, 'Missing nominal V2 value!'
abs_V2 = GTC.ureal(val2, val2/1e4, 8, label='abs_V2')
...
while Data_row <= Data_stop_row:
    ...
    Vlin_pert = I_INFO[role_descr['DVMd']]['linearity_pert']  # lin in G calc
    Vlin_Vdav = I_INFO[role_descr['DVMd']]['linearity_Vdav']  # lin in Vd calc\
    ...
    influencies.extend([R2_0, R2alpha,
                        R2beta, R2gamma])  # R2 dependancies
    ...
    for r in range(Data_row, Data_row+4):  # Build list of 4 gmh / T-dvm reads
        ...
        assert ws_Data['S'+str(r)].value is not None, 'No R1 raw DVM \
        (temperature) data!'
        raw_dvm1 = ws_Data['S'+str(r)].value

        assert ws_Data['T'+str(r)].value is not None, 'No R2 raw DVM \
        (temperature) data!'
    ...
    '''
    Data are plain numbers (with digitization rounding), so use
    ta.estimate_digitized() to return a ureal
    '''
...
#        influencies.append(T2_av_dvm,T2_av_gmh) # R2 dependancy
    ...
    # T-def due to imperfect positioning of both probes AND their disagreement:
    ...
    # All measurements have high-enough precision so ignore digitization error.
    V1 = []
    V2 = []
    Vd = []
    for line in range(4):
    ...
    # Define drift
    drift_err = Vd[2]-(Vd[0]+((Vd[3]-Vd[2])/(V2[3]-V2[2]))*(V2[2]-V2[0]))/4
    Vdrift1 = GTC.ureal(0, GTC.tb.distribution['gaussian'](abs(drift_err)), 8,
                        label='Vdrift_pert ' + Run_Id)
    Vdrift2 = GTC.ureal(0, GTC.tb.distribution['gaussian'](abs(drift_err)), 8,
    ...
    # NOTE: TWO abs() NEEDED TO ENSURE NON-NEGATIVE DIFFERENCE!:
    ...
    '''
    Combine data for this measurement:
    name, time, R, T, V and write to Summary sheet.
    '''
    ...
    summary_row = R_info.WriteBudget(ws_Summary, summary_row,
                                     budget_table_sorted)
    summary_row += 1  # Blank line between each measurement for ease of reading
