...
"""
os.environ['GMHPATH'] = 'I:\MSL\Private\Electricity\Staff\TBL' +\
    '\Python\High_Res_Bridge\GMHdll'
...
class GMH_Sensor(device):
    ...
    def Close(self):
        ...
        print 'devices.GMH_Sensor.Close(): CloseCom err_msg: ' +\
    ...
    def Transmit(self, Addr, Func):
        ...
        """
        self.error_code = \
            ct.c_int16(GMHLIB.GMH_Transmit(Addr, Func, ct.byref(self.Prio),
    ...
    def GetSensorInfo(self):
        ...
        for Address in range(1, 100):
            ...
            if self.Transmit(Addr, self.MeasFn):  # Result to self.intData
                ...
                if Address > 1:  # Don't let last address tried screw it up.
                    ...
                    self.demo = True
                # Assume all functions are in a contiguous address range,
                # starting at 1:
                break
    ...
    def Measure(self, meas='T'):
        """
        Measure either temperature, pressure or humidity, based on param meas
        Returns a float.
...
class instrument(device):
    '''
    A class for associating instrument data with a VISA instance
    of that instrument.
    '''
    def __init__(self, descr, demo=True):  # Default to demo mode
        ...
        assert self.Descr in INSTR_DATA,\
            'Unknown instrument - check instrument data is loaded from ' +\
...
#            INSTR_DATA[self.Descr]['demo'] is False  # A real working instr
            ...
            print 'devices.instrument.Open() failed:',\
                self.Descr, 'opened in demo mode'
        return self.instr

    def Close(self):
        # Close comms with instrument
        if self.demo is True:
            print 'devices.instrument.Close():',\
                self.Descr, 'in demo mode - nothing to close'
        if self.instr is not None:
            print 'devices.instrument.Close():',\
                self.Descr, 'session handle=', self.instr.session
            self.instr.close()
        else:
            print 'devices.instrument.Close():',\
                self.Descr, 'is "None" or already closed'
        self.is_open = 0

    def Init(self):
        # Send initiation string
        if self.demo is True:
            print 'devices.instrument.Init():',\
                self.Descr, 'in demo mode - no initiation necessary'
            return 1
        else:
            reply = 1
            for s in self.InitStr:
                if s != '':  # Instrument has an initiation string
                    try:
                        ...
                        return reply
            print 'devices.instrument.Init():',\
                self.Descr, 'initiated with cmd:', s
        return reply
    ...
    def SetV(self, V):
        ...
        if self.demo is True:
            ...
            try:
                ...
                print'Failed to write "%s" to %s,via handle %s' % \
    ...
    def CheckErr(self):
        ...
        if 'F5520A' in self.Descr:
            ...
            print'devices.instrument.CheckErr(): ' +\
                'Invalid function for', self.Descr
            return -1
    ...
    def SendCmd(self, s):
        ...
        if any(x in s for x in'?X'):
            print'devices.instrument.SendCmd(): ' +\
                'Query(%s) to %s' % (s, self.Descr)
            reply = self.instr.query(s)
            return reply
        elif s == '':
            reply = self.instr.read()
            print'devices.instrument.SendCmd(): Read()',\
                reply, 'from', self.Descr
            return reply
        else:
            print'devices.instrument.SendCmd(): ' +\
                'Write(%s) to %s' % (s, self.Descr)
            self.instr.write(s)
            return reply
